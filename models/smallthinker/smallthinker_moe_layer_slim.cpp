 ...
 * @date   12 May 2026
 * @brief  SmallThinker MoE layer with on-demand expert loading.
 */
...
void deactivate_if_needed(nntrainer::Tensor &tensor, bool active) {
  if (active)
    tensor.deactivate();
}
...
void SmallThinkerSlimMoELayer::run_active_expert(
  ...
  try {
    ...
    deactivate_if_needed(expert_down_proj, down_active);
    deactivate_if_needed(expert_up_proj, up_active);
    deactivate_if_needed(expert_gate_proj, gate_active);
    throw;
  }
