...
void SmallThinkerCachedSlimMoELayer::finalize(
  ...
  cache_size = std::min(num_experts, std::max(cache_size, topk));
...
void SmallThinkerCachedSlimMoELayer::forwarding(
  ...
  route_and_compute(context, input, router_input, output, router_logits,
                    expert_mask, hidden_size, total_tokens);
...
void SmallThinkerCachedSlimMoELayer::route_and_compute(
  nntrainer::RunLayerContext &context, nntrainer::Tensor &input,
  nntrainer::Tensor &router_input, nntrainer::Tensor &output,
  nntrainer::Tensor &router_logits, nntrainer::Tensor &expert_mask,
  unsigned int hidden_size, unsigned int total_tokens) {

  nntrainer::Tensor &gate_weights = context.getWeight(gate_idx);
  router_input.dot(gate_weights, router_logits);

  const unsigned int prefetch_k = std::min(num_experts, topk + 5);
  auto prefetch_result = router_logits.topK(prefetch_k);
  auto prefetch_indices = std::get<1>(prefetch_result);
  ...
  const uint32_t *prefetch_data = prefetch_indices.getData<uint32_t>();
  for (int i = static_cast<int>(total_tokens) - 1; i >= 0; --i) {
    for (int k = 0; k < static_cast<int>(prefetch_k); ++k) {
      const int expert_idx = prefetch_data[i * prefetch_k + k];
  ...
  std::vector<unsigned int> target_experts;
  target_experts.reserve(num_experts);
  for (unsigned int expert_idx = 0; expert_idx < num_experts; ++expert_idx) {
    if (!expert_assignments[expert_idx].empty())
      target_experts.push_back(expert_idx);
  }

  for (unsigned int expert_idx : target_experts) {
    activate_or_touch_expert(context, expert_idx);

    compute_expert_forward(
      input, output, expert_assignments[expert_idx],
...
void SmallThinkerCachedSlimMoELayer::activate_or_touch_expert(
...
void SmallThinkerCachedSlimMoELayer::evict_experts(
  ...
  while (true) {
    int expert_idx = -1;
    {
      ...
      expert_idx = loaded_expert_deque.front();
      loaded_expert_deque.pop_front();
      iteration_map.erase(expert_idx);
      need_load[expert_idx] = true;
    }

    deactivate_expert(context, expert_idx);
  }
}

void SmallThinkerCachedSlimMoELayer::deactivate_expert(
  nntrainer::RunLayerContext &context, unsigned int expert_idx) {
...
inline void SmallThinkerCachedSlimMoELayer::compute_expert_forward(
  ...
  for (size_t i = 0; i < num_tokens; ++i) {
    ...
    nntrainer::Tensor token_output =
      output.getSharedDataTensor(out_step_dim, token_idx * hidden_size, true);
